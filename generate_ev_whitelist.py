...
## SwissSign - AffirmTrust (vice versa: chain terminating at AffirmTrust includes SwissSign policy).
...
for chash, policies in ev_roots:
...
def ev_match(certificate, entry_type, extra_data, certificate_index):
    ...
    try:
        for policy in certificate.policies():
            if policy['policyIdentifier'] in EV_POLICIES:
                """
                h = hashlib.sha256()
